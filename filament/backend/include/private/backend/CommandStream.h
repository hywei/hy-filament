...
{
    static_assert(std::is_base_of<T, std::decay_t<D>>::value, "member function and object not related");
...
{
    return trampoline(std::forward<M>(m),
                      std::forward<D>(d),
                      std::forward<T>(t),
...
#define DECL_DRIVER_API_SYNCHRONOUS(RetType, methodName, paramsDecl, params) \
    ...
    { \
        ...
        AutoExecute callOnExit([=]() { DEBUG_COMMAND_END(methodName, true); }); \
...
public:
     ...
     */
    template<typename PodType, typename = typename std::enable_if<std::is_trivially_destructible<PodType>::value>::type>
    inline PodType* allocatePod(size_t count = 1, size_t alignment = alignof(PodType)) noexcept;
