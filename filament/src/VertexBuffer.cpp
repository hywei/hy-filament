                                                        ...
                                                        uint8_t         byteStride) noexcept
{
    ...
    if (size_t(attribute) < MAX_VERTEX_ATTRIBUTE_COUNT && size_t(bufferIndex) < MAX_VERTEX_ATTRIBUTE_COUNT)
    {
        ...
        if (byteOffset & 0x3u)
        {
            utils::slog.d << "[performance] VertexBuffer::Builder::attribute() "
                             "byteOffset not multiple of 4"
                          << utils::io::endl;
        }
        if (byteStride & 0x3u)
        {
            utils::slog.d << "[performance] VertexBuffer::Builder::attribute() "
                             "byteStride not multiple of 4"
                          << utils::io::endl;
        }
#endif
    ...
    {
        utils::slog.w << "Ignoring VertexBuffer attribute, the limit of " << MAX_VERTEX_ATTRIBUTE_COUNT
                      << " attributes has been exceeded" << utils::io::endl;
    }
    return *this;
}

VertexBuffer::Builder& VertexBuffer::Builder::normalized(VertexAttribute attribute, bool normalized) noexcept
{
...
VertexBuffer* VertexBuffer::Builder::build(Engine& engine)
{
    ...
    if (!ASSERT_PRECONDITION_NON_FATAL(mImpl->mBufferCount <= MAX_VERTEX_BUFFER_COUNT,
                                       "bufferCount cannot be more than %d",
    ...
    for (size_t vertAttriIndex = 0; vertAttriIndex < MAX_VERTEX_ATTRIBUTE_COUNT; ++vertAttriIndex)
    {
        if (declaredAttributes[vertAttriIndex])
        {
            attributedBuffers.set(attributes[vertAttriIndex].buffer);
        }
    }
    ...
    mBufferObjectsEnabled(builder->mBufferObjectsEnabled)
{
    ...
    static_assert(sizeof(Attribute) == sizeof(AttributeData), "Attribute and Builder::Attribute must match");
...
#pragma nounroll
        for (size_t i = 0; i < MAX_VERTEX_BUFFER_COUNT; ++i)
        {
            if (bufferSizes[i] > 0)
            {
                BufferObjectHandle bo = driver.createBufferObject(bufferSizes[i], backend::BufferObjectBinding::VERTEX);
...
void FVertexBuffer::setBufferAt(FEngine&                    engine,
                                uint8_t                     bufferIndex,
                                ...
                                uint32_t                    byteOffset)
{
    ...
    if (bufferIndex < mBufferCount)
    {
        ...
        engine.getDriverApi().updateBufferObject(mBufferObjects[bufferIndex], std::move(buffer), byteOffset);
    }
    else
    {
...
void FVertexBuffer::setBufferObjectAt(FEngine& engine, uint8_t bufferIndex, FBufferObject const* bufferObject)
{
...
void VertexBuffer::setBufferAt(Engine&                     engine,
                               uint8_t                     bufferIndex,
                               backend::BufferDescriptor&& buffer,
                               uint32_t                    byteOffset)
{
...
void VertexBuffer::setBufferObjectAt(Engine& engine, uint8_t bufferIndex, BufferObject const* bufferObject)
{
